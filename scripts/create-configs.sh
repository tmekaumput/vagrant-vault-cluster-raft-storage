...
function config_cluster_node {

  rm -f vault.hcl

  sudo cp /tmp/license.txt /etc/vault.d/license.txt

  IFS=', ' read -r -a LEADER_API_ADDRESSES_ARR <<< "$LEADER_API_ADDRESSES"

  NEWLINE=$'\n'
  for LEADER_API_ADDRESS in "${LEADER_API_ADDRESSES_ARR[@]}"
  do

    read -r -d '' JOIN_CLAUSE << EOM
    retry_join {
      leader_api_addr = "${LEADER_API_ADDRESS}"
    }
EOM

    JOIN_CLAUSES="${JOIN_CLAUSE}${NEWLINE}${JOIN_CLAUSES}"
